...
execute as @e[dx=0,tag=!mcrpg_caster] positioned ~-.99 ~-.99 ~-.99 if entity @s[dx=0] run function mcrpg:skills/fire/subfunc/sf_dmg

scoreboard players add @s mcrpg_rcdist 1

# block collision animation
execute unless block ~ ~ ~ #mcrpg:no_collision run particle soul_fire_flame ~ ~ ~ 0 0 0 .3 128 normal @a[tag=!mcrpg_ldm]
