...
execute as @e[dx=0,tag=!caster,type=!#mcrpg:no_xp] positioned ~-.99 ~-.99 ~-.99 if entity @s[dx=0] run function mcrpg:skills/water/subfunc/water_beam/hit

# do not bounce on fluids 
execute positioned ^ ^ ^0.25 if block ~ ~ ~ fire run function mcrpg:skills/water/subfunc/water_beam/extinguish
execute if score %bounce mcrpg matches 1.. if block ^ ^ ^0.25 #mcrpg:fluids run function mcrpg:skills/water/subfunc/water_beam/absorb
